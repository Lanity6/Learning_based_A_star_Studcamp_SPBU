...
class Autoencoder(pl.LightningModule):
    ...
    def step(self, batch, batch_idx, regime):
        ...
        inputs = torch.cat([map_design, start + goal], dim=1) if self.mode in ('f', 'nastar') else torch.cat(
            [map_design, goal], dim=1)
        predictions = self(inputs)
        mask = build_loss_mask(map_design, start, goal, self.mode)
        loss_raw = self.recon_criterion((predictions + 1) / 2 * self.k, gt_hmap)
