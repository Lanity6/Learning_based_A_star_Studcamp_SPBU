...
def main(mode, state_dict_path, hardness_limit=1.05):
    ...
    for batch in tqdm(test_dataloader):
        with torch.no_grad():
            ...
            inputs = torch.cat([map_design, start + goal], dim=1) if mode == 'f' else torch.cat([map_design, goal],
                                                                                                dim=1)
            ...
            expansions_ratio.append(
                ((learn_outputs.histories).sum((-1, -2, -3))) / ((vanilla_outputs.histories).sum((-1, -2, -3))))
