from models.autoencoder_with_loss_mask import Autoencoder, PathLogger
...
def main(mode, run_name, proj_name, batch_size, max_epochs):
    ...
    checkpoint_callback = ModelCheckpoint(
        dirpath="checkpoints/with_loss_mask",
        filename="best",
        save_top_k=1,
        monitor="val_loss",
        mode="min"
    ...
    if best_ckpt:
        ...
        if mode == "f":
            torch.save(state_dict, "weights/best_weights_with_loss_mask_f.pth")
            print("Saved best model to best_weights_with_loss_mask_f.pth")
        else:
            torch.save(state_dict, "weights/best_weights_with_loss_mask_cf.pth")
            print("Saved best model to best_weights_with_loss_mask_cf.pth")
    wandb.finish()
